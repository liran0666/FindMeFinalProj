...
export function GalleryPage({ isCustomer = false }) {
  ...
  //בחירת הכל
  const selectAll = () =>
    setSelected(new Set(displayedPhotos.map((p) => p.filename)));
  ...
  return (
    <div className={styles.page}>
      ...
      <button
        ...
        onClick={() =>
          navigate(isCustomer ? "/customer/events" : "/photographer/events")
        }
      >
        ← חזרה לאירועים
      </button>
      ...
      <div className={styles.pageHeader}>
        <div>
          ...
          <div className={styles.pageSubtitle}>
            {isCustomer
              ? "התמונות שלך מהאירוע"
              : `אירוע #${eventId} · ${photos.length} תמונות`}
          </div>
        </div>
        {!isCustomer && (
          <button
        ...
        <>
          <div className={styles.sharedBanner}>
            ...
            <div className={styles.sharedBannerText}>
              <div className={styles.sharedBannerTitle}>
                התמונות שלך מוכנות!
              </div>
              <div className={styles.sharedBannerSub}>
                הצלם העלה {photos.length} תמונות עבורך
              </div>
            </div>
          </div>
          ...
          <div className={styles.faceFilterPanel}>
            <div className={styles.faceFilterLeft}>
              ...
              <div className={styles.faceFilterInfo}>
                ...
                <div className={styles.faceFilterSub}>
                  {faceFilterMsg ||
                    "העלה סלפי ונמצא את כל התמונות שבהן הפנים שלך מופיעות"}
                </div>
              </div>
            </div>
            <div className={styles.faceFilterActions}>
              ...
              <button
                ...
                disabled={faceFiltering}
              >
                {faceFiltering ? (
                  <span className={styles.filteringSpinner}>⏳ מנתח...</span>
                ) : (
                  "📸 העלה סלפי"
                )}
              </button>
        ...
        <div className={styles.emptyState}>
          ...
          <div className={styles.emptyText}>
            {filteredPhotos !== null
              ...
              : isCustomer
                ? "הצלם טרם העלה תמונות לאירוע זה"
                : "העלה תמונות לאירוע זה"}
          </div>
        </div>
      )}
                ...
                >
                  ...
                  <div className={styles.photoOverlay}>
                    <button
                      ...
                      onClick={() => setLightbox(`${SERVER}${photo.url}`)}
                    >
                      🔍
                    </button>
                    <button
                      ...
                      onClick={async (e) => {
                        e.stopPropagation();
                        const blob = await fetch(`${SERVER}${photo.url}`).then(
                          (r) => r.blob(),
                        );
                        const blobUrl = URL.createObjectURL(blob);
                        const a = document.createElement("a");
      ...
      {selected.size > 0 && (
        <div className={styles.selectionBar}>
          ...
          <div className={styles.selectionActions}>
            ...
            <button
              className={styles.selectionDownloadBtn}
