...
function getStatusInfo(ev) {
  ...
  if (ev.status === "pending")
    return { label: "ממתין לאישור", cls: "pending", step: 1 };
  if (ev.status === "declined")
    return { label: "נדחה", cls: "declined", step: 0 };
  if (ev.status === "active" && !past)
    return { label: "אושר — קרוב", cls: "active", step: 2 };
  if (ev.status === "active" && past)
    return { label: "הושלם", cls: "done", step: 3 };
...
export default function CustomerEventsPage() {
  ...
  const handleRated = (eventId, rating) => {
    setEvents((prev) =>
      prev.map((e) =>
        e.id === eventId ? { ...e, customer_rating: rating } : e,
      ),
    );
    setRatingEvent(null);
    // //מרפרש פרטים אם נפתח
    setDetailsEvent((prev) =>
      prev?.id === eventId ? { ...prev, customer_rating: rating } : prev,
    );
  };
  ...
  const counts = {
    ...
    active: events.filter((e) => e.status === "active" && !isPast(e.date))
      .length,
      ...
      {!loading && events.length > 0 && (
        <div className={styles.summaryRow}>
          {[
            ...
            <div
              key={i}
              className={`${styles.summaryCard} ${styles[`color_${s.color}`]}`}
            >
      ...
      {loading ? (
        ...
        <div className={styles.emptyState}>
          ...
          <div className={styles.emptyText}>
            עבור לחיפוש צלמים ושלח הצעת אירוע
          </div>
        </div>
      ) : filtered.length === 0 ? (
        <div className={styles.emptyState}>
    ...
    <div className={`${styles.card} ${styles[`card_${cls}`]}`}>
      ...
      <div className={styles.cardBody}>
        <div className={styles.cardTop}>
          ...
          <span className={`${styles.statusPill} ${styles[`pill_${cls}`]}`}>
            {label}
          </span>
        </div>
          ...
          {["נשלחה", "אושרה", "הסתיים"].map((s, i) => {
            ...
            return (
              <div key={i} className={styles.miniStep}>
                <div
                  className={`${styles.miniDot} ${active ? styles.miniDotOn : ""} ${current ? styles.miniDotCurrent : ""}`}
                />
                <div
                  className={`${styles.miniLabel} ${active ? styles.miniLabelOn : ""}`}
                >
                  {s}
                </div>
                {i < 2 && (
                  <div
                    className={`${styles.miniLine} ${step > i + 1 ? styles.miniLineOn : ""}`}
                  />
                )}
              </div>
            );
      ...
      <div className={styles.cardActions}>
        ...
        {step >= 2 && (
          <button className={styles.galleryBtn} onClick={onGallery}>
            🖼️ גלריה
          </button>
        )}
        {canRate && (
      ...
      <div className={styles.detailsModal} onClick={(e) => e.stopPropagation()}>
              ...
              return (
                <div key={i} className={styles.timelineStep}>
                  <div className={styles.timelineLeft}>
                    ...
                    {i < 2 && (
                      <div
                        className={`${styles.timelineConnector} ${done ? styles.timelineConnectorDone : ""}`}
                      />
                    )}
                  </div>
                  <div className={styles.timelineContent}>
                    <div
                      className={`${styles.timelineLabel} ${pending ? styles.timelineLabelPending : ""}`}
                    >
                      {s.label}
                    </div>
        ...
        {step === 3 && ev.customer_rating !== null && (
          <div className={styles.detailsRated}>
            ...
            <div className={styles.detailsRatedStars}>
              {"⭐".repeat(ev.customer_rating)}
            </div>
          </div>
        )}
  ...
  return (
    <div className={styles.overlay} onClick={onClose}>
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.modalEventName}>
